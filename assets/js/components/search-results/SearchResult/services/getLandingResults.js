...
export default function getLandingResults(ckanUrl, phrase, year) {
  ...
  const request = new Promise((resolve, reject) => {
    const urlsArray = [
      `${ckanUrl}/api/3/action/package_search?q=${encodeURI(phrase)}&start=0&rows=3&fq=+organization:national-treasury+vocab_financial_years:${year}+groups:"budget-vote-documents"+extras_department_name_slug:[*%20TO%20*]+extras_geographic_region_slug:[*%20TO%20*]&ext_highlight=true`,

      `${ckanUrl}/api/3/action/package_search?q=${encodeURI(phrase)}&start=0&rows=0&fq=+organization:national-treasury+groups:"budget-vote-documents"+extras_department_name_slug:[*%20TO%20*]+extras_geographic_region_slug:[*%20TO%20*]&facet.field=[%22vocab_financial_years%22]`,

      `${ckanUrl}/api/3/action/package_search?q=${encodeURI(phrase)}&start=0&rows=3&fq=-organization:national-treasury%20AND%20NOT%20groups:[%22%22%20TO%20*]&ext_highlight=true`,

      `${ckanUrl}/api/3/action/package_search?q=${encodeURI(phrase)}&start=0&rows=3&fq=groups:[%22%22%20TO%20*]-groups:"budget-vote-documents"-groups:"adjusted-budget-vote-documents"&ext_highlight=true`,
