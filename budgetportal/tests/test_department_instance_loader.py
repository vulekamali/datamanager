...
from budgetportal.import_export_admin import DepartmentInstanceLoader, DepartmentResource
...
class DepartmentInstanceLoaderTest(TestCase):
    ...
    def setUp(self):
        ...
        fields = ['government', 'department_name',
                  'is_vote_primary', 'vote_number']
        self.dataset = Dataset(headers=fields)

    def test_get_instance_by_name(self):
        row = [self.government.name, self.department.name,
               "False", self.department.vote_number]
        self.dataset.append(row)
        self.instance_loader = DepartmentInstanceLoader(
    ...
    def test_get_instance_by_slug(self):
        row = [self.government.name, self.department.name.upper(), "False",
