...
function notAvailableMessage() {
    return (
        <div>
            <p className="u-fontStyle u-fontStyle--italic">This data is not yet available. Provincial budgets are tabled
                after the national budget has been announced. This is because the national budget determines the amount
                of money each province receives. We expect to be able make provincial budget data available by
                April {(new Date()).getFullYear()}.</p>
...
function departments(linksArray, doubleRow) {
    return (
        <ul className={`DeptGroup-list${doubleRow ? ' DeptGroup-list--doubleRow' : ''}`}>
            {
                linksArray.map(({name, url_path: url}, index) => {
                    return (
                        <li
