...
class Q1Q2Q3UpdateTestCase(TransactionTestCase):
    def setUp(self):
        self.file = open(EMPTY_FILE_PATH)
        self.file_2 = open(EMPTY_FILE_PATH)
        self.file_3 = open(EMPTY_FILE_PATH)
        self.file_4 = open(EMPTY_FILE_PATH)
    ...
    def test_q1_q2_q3_updated_after_q4_snapshot_inserted(self):
        ...
        # Check Q3 values
        self.assertEqual(snapshots_data[2]["total_spent_in_quarter"], 30)
        self.assertEqual(snapshots_data[2]["total_spent_to_date"], 263)

        # Create Q4 snapshot
        q4 = Quarter.objects.create(number=4)
        irm_snapshot_4 = IRMSnapshot.objects.create(
            financial_year=self.fin_year,
            quarter=q4,
            date_taken=date(2031, 3, 31),
            file=File(self.file_4),
        )
        ProvInfraProjectSnapshot.objects.create(
            irm_snapshot=irm_snapshot_4,
            project=self.project,
            actual_expenditure_q1=13,
            actual_expenditure_q2=22,
            actual_expenditure_q3=31,
            actual_expenditure_q4=40,
            expenditure_from_previous_years_total=200,
        )
        # Recreate the chart data
        snapshots_data = time_series_data(self.project.project_snapshots.all())
        ...
        # Check Q1 values
        self.assertEqual(snapshots_data[0]["total_spent_in_quarter"], 13)
        self.assertEqual(snapshots_data[0]["total_spent_to_date"], 213)

        # Check Q2 values
        self.assertEqual(snapshots_data[1]["total_spent_in_quarter"], 22)
        self.assertEqual(snapshots_data[1]["total_spent_to_date"], 235)

        # Check Q3 values
        self.assertEqual(snapshots_data[2]["total_spent_in_quarter"], 31)
        self.assertEqual(snapshots_data[2]["total_spent_to_date"], 266)
