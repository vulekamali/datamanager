...
class ExportImportDepartmentsTestCase(TestCase):
    def setUp(self):
        ...
        for sphere_slug in self.sphere_slugs:
            # spheres
            self.spheres[sphere_slug] = Sphere.objects.create(financial_year=self.year, name=sphere_slug)

            # governments
            self.governments[sphere_slug] = Government.objects.create(sphere=self.spheres[sphere_slug], name='South Africa')
    ...
    def test_load_all_departments_from_export(self):
        ...
        for sphere_slug in self.sphere_slugs:
            with NamedTemporaryFile() as csv_file:
                ...
                dept_2 = Department.objects.get(government=self.governments[sphere_slug], vote_number=2)
