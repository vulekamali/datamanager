...
VALID_REPORT_TYPES = ['Provincial Institutions Oversight Performance  Report','National Institutions Oversight Performance  Report']

def generate_import_report(report_type_validated, not_matching_departments, primary_key_violations):
    report = ""
    ...
    if len(primary_key_violations) > 0:
        report += "Cannot create multiple elements with the same department & indicator names : " + os.linesep
        for violation in primary_key_violations:
            report += f"* department name : {violation[0]} / indicator : {violation[1]}" + os.linesep
    return report
...
def save_imported_indicators(parsed_data, obj_id):
    ...
    for indicator_data in parsed_data:
        ...
        existing_obj = models.Indicator.objects.filter(department=department_obj, indicator_name=indicator_name)
        ...
        if len(existing_obj) <= 0:
            ...
            violation = [existing_obj[0].department.name, existing_obj[0].indicator_name]
            if violation not in primary_key_violations:
                primary_key_violations.append(violation)
    ...
    obj_to_update.import_report = generate_import_report(True, not_matching_departments, primary_key_violations)
    obj_to_update.save()
...
def validate_report_type(full_text, obj_id):
    ...
    if not validated:
        obj_to_update = models.EQPRSFileUpload.objects.get(id=obj_id)
        obj_to_update.import_report = generate_import_report(False, [], [])
        obj_to_update.save()
