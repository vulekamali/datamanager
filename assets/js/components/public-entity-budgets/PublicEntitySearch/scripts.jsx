...
class PublicEntitySearchContainer extends React.Component {
  constructor(props) {
    ...
    this.state = {
      loading: false,
      open: null,
      results: filterResults(filters, Object.keys(this.props.publicEntities[0]).map(key => this.props.publicEntities[0][key])),
      filters,
    };
  ...
  updateKeywords(keywords) {
    ...
    this.setState({ results: filterResults(filters, Object.keys(this.props.publicEntities[0]).map(key => this.props.publicEntities[0][key])) });
  }
  ...
  updateDropdown(filter, value) {
    ...
    this.setState({ filters });
    return this.setState({ results: filterResults(filters, Object.keys(this.props.publicEntities[0]).map(key => this.props.publicEntities[0][key])) });
  }

  render() {
    return <PublicEntitySearch state={this.state} eventHandlers={this.eventHandlers} />;
  }
}


function scripts() {
  const componentsList = document.getElementsByClassName('js-initPublicEntitySearch');
  const ckanUrl = document.getElementsByTagName('body')[0].getAttribute('data-ckan-url');;


  for (let i = 0; i < componentsList.length; i++) {
    const component = componentsList[i];
    const publicEntitiesData = JSON.parse(decodeHtmlEntities(component.getAttribute('data-public-entities-json'))).data;
    ...
    ReactDOM.render(
      <PublicEntitySearchContainer {...{ publicEntities, ckanUrl, financialYear, department, functiongroup1, phrase }} />,
      component,
    );
  }
}
