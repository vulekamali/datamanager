...
class FinancialYearPage(View):
    ...
    def get(self, request, financial_year_id):
        ...
        response_yaml = yaml.safe_dump(
            context, default_flow_style=False, encoding="utf-8"
        )
        return HttpResponse(response_yaml, content_type="text/x-yaml")
...
def infrastructure_projects_overview(request):
    ...
    infrastructure_projects = (
        InfrastructureProject.get_featured_projects_from_resource()
    )
    ...
    for project in infrastructure_projects:
        departments = Department.objects.filter(
            slug=slugify(project.department_name), government__sphere__slug="national"
        )
        department_url = None
        if departments:
            department_url = (
        ...
        projects.append(
            {
                ...
                "expenditure": sorted(
                    project.complete_expenditure, key=lambda e: e["year"]
                ),
            }
        )
